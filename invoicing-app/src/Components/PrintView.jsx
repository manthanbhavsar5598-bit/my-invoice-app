...
function buildInvoiceHTML(invoice, business, client) {
  ...
  const gstRows = isIntra
    ? `<tr style="background:#FBEEDD;">
        <td style="padding:7px 10px;font-size:14px;font-weight:700;color:#111827;border-bottom:1px solid #1F2A3C;">SGST ${(Number(invoice.taxRate) / 2).toFixed(1)}%</td>
        <td style="padding:7px 10px;font-size:12px;font-weight:700;color:#111827;border-bottom:1px solid #1F2A3C;">${symbol}</td>
        <td style="padding:7px 10px;font-size:14px;font-weight:700;color:#111827;border-bottom:1px solid #1F2A3C;text-align:right;font-family:'IBM Plex Mono',monospace;">${halfTax.toFixed(2)}</td>
      </tr>
      <tr style="background:#FBEEDD;">
        <td style="padding:7px 10px;font-size:14px;font-weight:700;color:#111827;border-bottom:1px solid #1F2A3C;">CGST ${(Number(invoice.taxRate) / 2).toFixed(1)}%</td>
        <td style="padding:7px 10px;font-size:12px;font-weight:700;color:#111827;border-bottom:1px solid #1F2A3C;">${symbol}</td>
        <td style="padding:7px 10px;font-size:14px;font-weight:700;color:#111827;border-bottom:1px solid #1F2A3C;text-align:right;font-family:'IBM Plex Mono',monospace;">${halfTax.toFixed(2)}</td>
      </tr>`
    : isInter
    ? `<tr style="background:#FBEEDD;">
        <td style="padding:7px 10px;font-size:14px;font-weight:700;color:#111827;border-bottom:1px solid #1F2A3C;">IGST ${Number(invoice.taxRate).toFixed(1)}%</td>
        <td style="padding:7px 10px;font-size:12px;font-weight:700;color:#111827;border-bottom:1px solid #1F2A3C;">${symbol}</td>
        <td style="padding:7px 10px;font-size:14px;font-weight:700;color:#111827;border-bottom:1px solid #1F2A3C;text-align:right;font-family:'IBM Plex Mono',monospace;">${totals.taxAmount.toFixed(2)}</td>
      </tr>`
    : "";
...
<style>
  ...
  * { box-sizing: border-box; }
  body { font-family: 'Inter', sans-serif; color: #111827; background: #fff; margin: 0; padding: 24px; -webkit-print-color-adjust: exact; print-color-adjust: exact; color-adjust: exact; }
...
<body>
  ...
  <div class="sheet">
    <div style="text-align:center;font-weight:700;font-size:13px;letter-spacing:0.06em;padding:6px 0;border-bottom:1px solid #1F2A3C;color:${commission ? "#C0392B" : "#3c1f1f"};">
      JAY SWAMINARAYAN
    </div>
    <div style="background:#fff;border-bottom:1px solid #1F2A3C;padding:12px 16px;text-align:center;">
      <span style="font-family:'Zilla Slab',serif;font-size:22px;font-weight:700;background:#FFD966;padding:3px 14px;display:block;margin-left:-16px;margin-right:-16px;">${escapeHtml(business.name)}</span>
      ...
      ${business.gstNumber ? `<div style="font-size:13px;font-weight:700;text-transform:uppercase;margin-top:4px;">GSTIN: ${escapeHtml(business.gstNumber)}</div>` : ""}
      ${business.phone ? `<div style="font-size:13px;font-weight:700;text-transform:uppercase;margin-top:3px;">Mobile: ${escapeHtml(business.phone)}</div>` : ""}
      ${business.email ? `<div style="font-size:13px;font-weight:700;text-transform:uppercase;margin-top:3px;">Email: ${escapeHtml(business.email)}</div>` : ""}
    </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;">
      <div style="border:1px solid #1F2A3C;border-right:0;padding:7px 8px;font-size:15px;font-weight:800;color:#111827;"><b>Invoice no:</b> ${escapeHtml(invoice.number)}</div>
      <div style="border:1px solid #1F2A3C;border-left:0;padding:7px 8px;font-size:15px;font-weight:800;color:#111827;text-align:right;"><b>Date:</b> ${fmtDate(invoice.issueDate)}</div>
    </div>` : `
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;">
      <div style="border:1px solid #1F2A3C;border-right:0;padding:7px 8px;font-size:15px;font-weight:800;color:#111827;"><b>Invoice no:</b> ${escapeHtml(invoice.number)}</div>
      <div style="border:1px solid #1F2A3C;border-left:0;border-right:0;padding:7px 8px;font-size:15px;font-weight:800;color:#111827;text-align:center;"><b>State:</b> ${isIntra ? "Intra-state" : isInter ? "Inter-state" : "N/A"}</div>
      <div style="border:1px solid #1F2A3C;border-left:0;padding:7px 8px;font-size:15px;font-weight:800;color:#111827;text-align:right;"><b>Date:</b> ${fmtDate(invoice.issueDate)}</div>
    </div>`}
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;border-top:1px solid #1F2A3C;">
      <div style="border-right:1px solid #1F2A3C;">
        <div style="padding:7px 8px;font-size:15px;font-weight:800;color:#111827;border-bottom:1px solid #1F2A3C;"><b>Bill to:</b> ${escapeHtml(client?.name || "—")}</div>
        <div style="padding:7px 8px;font-size:15px;font-weight:800;color:#111827;border-bottom:1px solid #1F2A3C;white-space:pre-line;"><b>Address:</b> ${escapeHtml(client?.address)}</div>
        <div style="padding:7px 8px;font-size:15px;font-weight:800;color:#111827;border-bottom:1px solid #1F2A3C;"><b>GST no:</b> ${escapeHtml(client?.gstNumber || "—")}</div>
        <div style="padding:7px 8px;font-size:15px;font-weight:800;color:#111827;"><b>State code:</b> ${escapeHtml(client?.stateCode || "—")}</div>
      </div>
      <div>
        <div style="padding:7px 8px;font-size:15px;font-weight:800;color:#111827;border-bottom:1px solid #1F2A3C;"><b>${invoice.shipDispatchType === "shipTo" ? "Ship to:" : "Dispatch from:"}</b> ${escapeHtml(invoice.shipDispatchName)}</div>
        <div style="padding:7px 8px;font-size:15px;font-weight:800;color:#111827;border-bottom:1px solid #1F2A3C;white-space:pre-line;"><b>Address:</b> ${escapeHtml(invoice.shipDispatchAddress)}</div>
        <div style="padding:7px 8px;font-size:15px;font-weight:800;color:#111827;"><b>GST no:</b> ${escapeHtml(invoice.shipDispatchGst || "—")}</div>
      </div>
    </div>` : `
    <div style="border-top:1px solid #1F2A3C;">
      <div style="padding:7px 8px;font-size:15px;font-weight:800;color:#111827;border-bottom:1px solid #1F2A3C;"><b>${commission ? "Party name:" : "Bill to:"}</b> ${escapeHtml(client?.name || "—")}</div>
      <div style="padding:7px 8px;font-size:15px;font-weight:800;color:#111827;border-bottom:1px solid #1F2A3C;white-space:pre-line;"><b>Address:</b> ${escapeHtml(client?.address)}</div>
      <div style="padding:7px 8px;font-size:15px;font-weight:800;color:#111827;border-bottom:1px solid #1F2A3C;"><b>GST no:</b> ${escapeHtml(client?.gstNumber || "—")}</div>
      <div style="padding:7px 8px;font-size:15px;font-weight:800;color:#111827;"><b>State code:</b> ${escapeHtml(client?.stateCode || "—")}</div>
    </div>`}
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;border-top:1px solid #1F2A3C;">
      <div style="border:1px solid #1F2A3C;border-right:0;padding:7px 8px;font-size:14px;font-weight:700;color:#111827;"><b>Transport details:</b> ${escapeHtml(invoice.transportName || "—")}</div>
      <div style="border:1px solid #1F2A3C;border-left:0;padding:7px 8px;font-size:14px;font-weight:700;color:#111827;"><b>Vehicle no:</b> ${escapeHtml(invoice.vehicleNo || "—")}</div>
    </div>` : ""}
    ...
    <div style="display:grid;grid-template-columns:1.4fr 1fr;">
      <div style="border-right:1px solid #1F2A3C;">
        <!-- Amount in Words -->
        <div style="padding:9px 10px;font-size:15px;font-weight:800;color:#111827;border-bottom:1px solid #1F2A3C;">${escapeHtml(amountInWords(grandTotal, "Rupees"))}</div>
        
        <!-- Bank Details -->
        <div style="padding:9px 10px;border-bottom:1px solid #1F2A3C;color:#111827;font-weight:700;">
          <div style="font-weight:800;font-size:15px;margin-bottom:5px;color:#111827;">Bank details</div>
          <div style="font-size:14px;font-weight:700;color:#111827;">Bank name: ${escapeHtml(business.bankName || "—")}</div>
          <div style="font-size:14px;font-weight:700;color:#111827;">Branch name: ${escapeHtml(business.branchName || "—")}</div>
          <div style="font-size:14px;font-weight:700;color:#111827;">A/C no: ${escapeHtml(business.accountNo || "—")}</div>
          <div style="font-size:14px;font-weight:700;color:#111827;">IFSC code: ${escapeHtml(business.ifscCode || "—")}</div>
        </div>
        
        <!-- Terms & Conditions -->
        <div style="padding:8px 10px;">
          <div style="font-weight:800;font-size:15px;margin-bottom:5px;color:#111827;">Terms &amp; conditions</div>
          ${terms.map((t) => `<div style="font-size:13.5px;font-weight:600;color:#111827;margin-bottom:3px;">${escapeHtml(t)}</div>`).join("")}
        </div>
      </div>
      <div>
        <table>
          <tbody>
            <tr style="background:#FBEEDD;">
              <td style="padding:7px 10px;font-size:14px;font-weight:700;color:#111827;border-bottom:1px solid #1F2A3C;">Sub total</td>
              <td style="padding:7px 10px;font-size:12px;font-weight:700;color:#111827;border-bottom:1px solid #1F2A3C;width:20px;">${symbol}</td>
              <td style="padding:7px 10px;font-size:14px;font-weight:700;color:#111827;border-bottom:1px solid #1F2A3C;text-align:right;font-family:'IBM Plex Mono',monospace;">${totals.subtotal.toFixed(2)}</td>
            </tr>
            ${gstRows}
            <tr style="background:#FBEEDD;">
              <td style="padding:7px 10px;font-size:14px;font-weight:700;color:#111827;border-bottom:1px solid #1F2A3C;">Round off</td>
              <td style="padding:7px 10px;font-size:12px;font-weight:700;color:#111827;border-bottom:1px solid #1F2A3C;">${symbol}</td>
              <td style="padding:7px 10px;font-size:14px;font-weight:700;color:#111827;border-bottom:1px solid #1F2A3C;text-align:right;font-family:'IBM Plex Mono',monospace;">${roundOff.toFixed(2)}</td>
            </tr>
            <tr style="background:#F5D98C;">
              <td style="padding:9px 10px;font-size:14px;font-weight:800;color:#111827;">Grand total</td>
              <td style="padding:9px 10px;font-size:13px;font-weight:800;color:#111827;">${symbol}</td>
              <td style="padding:9px 10px;font-size:14px;font-weight:800;color:#111827;text-align:right;font-family:'IBM Plex Mono',monospace;">${grandTotal.toFixed(2)}</td>
            </tr>
          </tbody>
        </table>
...
export default function PrintView({ invoice, business, client, onClose }) {
  ...
  return (
    <div>
      ...
      <div className="lg-print-area" style={{ fontFamily: "Inter, sans-serif", color: "#111827", background: "#fff", maxWidth: 900, margin: "0 auto", border: "1px solid #1F2A3C" }}>
        ...
        <div style={{ background: "#fff", borderBottom: "1px solid #1F2A3C", padding: "12px 16px", textAlign: "center" }}>
          <span className="lg-display" style={{ fontSize: 22, fontWeight: 700, background: "#FFD966", padding: "3px 14px", display: "block", marginLeft: -16, marginRight: -16 }}>{business.name}</span>
          <div style={{ fontSize: 13, fontWeight: 700, textTransform: "uppercase", marginTop: 6, whiteSpace: "pre-line" }}>{business.address}</div>
          {business.gstNumber && <div style={{ fontSize: 13, fontWeight: 700, textTransform: "uppercase", marginTop: 4 }}>GSTIN: {business.gstNumber}</div>}
          {business.phone && <div style={{ fontSize: 13, fontWeight: 700, textTransform: "uppercase", marginTop: 3 }}>Mobile: {business.phone}</div>}
          {business.email && <div style={{ fontSize: 13, fontWeight: 700, textTransform: "uppercase", marginTop: 3 }}>Email: {business.email}</div>}
        </div>
